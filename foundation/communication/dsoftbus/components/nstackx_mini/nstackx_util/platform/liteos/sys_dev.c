...
static int32_t GetInterfaceInfo(int32_t fd, int32_t option, struct ifreq *interface)
{
    ...
    if (lwip_ioctl(fd, SIOCGIFFLAGS, (char*)interface) < 0) {
...
static int32_t GetInterfaceIP(int32_t fd, struct ifreq *interface)
{
...
static int32_t GetInterfaceList(struct ifconf *ifc, struct ifreq *buf, uint32_t size)
{
    ...
    if (lwip_ioctl(fd, SIOCGIFCONF, (char*)ifc) < 0) {
        LOGE(TAG, "lwip_ioctl fail, errno = %d", errno);
        CloseSocketInner(fd);
        return NSTACKX_EFAILED;
    }
...
static int32_t GetTargetInterface(const struct sockaddr_in *dstAddr, struct ifreq *localDev)
{
    struct ifreq buf[INTERFACE_MAX];
    struct ifconf ifc;
    uint32_t localIp;
